...
const previous = (() => {
  try {
    ...
    const generatedAt = src.match(/"generatedAt":\s*"([^"]+)"/)?.[1] ?? null;
    return { tests: n("tests"), testFiles: n("testFiles"), generatedAt, src };
  } catch {
    return { tests: 0, testFiles: 0, generatedAt: null, src: "" };
  }
})();
...
/**
 * generatedAt: the twin's own HEAD commit date, never `new Date()`. This node
 * sits in check-generated.mjs's byte-deterministic set — a wall-clock stamp
 * would disagree with the committed file every day the twin's HEAD has not
 * moved, which is most days. When the twin is not checked out, the previous
 * committed stamp carries over, same posture as twinBlock just above.
 */
const generatedAt = twin
  ? (execFileSync("git", ["log", "-1", "--format=%cs"], { cwd: KMP, env: gitEnv(), encoding: "utf8" }).trim() ||
      previous.generatedAt || new Date().toISOString().slice(0, 10))
  : (previous.generatedAt ?? new Date().toISOString().slice(0, 10));
