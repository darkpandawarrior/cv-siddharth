...
const titlesPath = join(root, "src", "data", "weebTitles.ts");
...
/**
 * The scatter's whole diet: matched AND scored shows only — name, year,
 * genres, crowd score, his score. Drops the 297 unscored "To Watch" rows and
 * every cover (REC-5): a genre scatter needs none of that, and shipping it
 * anyway is the 130 kB mistake this file exists to not repeat.
 */
const titles = shows
  .filter((s) => s.al && s.score > 0 && s.al.crowd != null)
  .map((s) => ({
    name: s.name,
    year: s.year,
    genres: s.al.genres.length ? s.al.genres : s.genres,
    crowd: s.al.crowd,
    mine: s.score,
  }));
...
writeFileSync(
  ...
  `// AUTO-GENERATED by scripts/gen-weeb.mjs. Do not edit by hand.\n` +
    ...
    // Same run, same date as weeb.ts's own stamp above — kept as a sibling
    // const rather than folding weebTitles into an object, so WeebGenres.tsx
    // keeps calling .map()/.filter() straight on the array (check-freshness.mjs
    // was blind to this file: no export here matched STAMP_RE at all).
    `export const weebTitlesGeneratedAt = ${JSON.stringify(out.generatedAt)};\n` +
    `export const weebTitles = ${JSON.stringify(titles, null, 2)} as const;\n`,
);
