...
async function main() {
  ...
  // Measured, not wall-clock: the sibling `PaymentsLab` checkout's own last
  // commit date, same idiom gen-repo-stats.mjs uses for the Kotlin twin's
  // stamp. A live
  // Date.now() here would change on every run regardless of whether the
  // docs actually moved, which defeats the no-op check below AND leaves
  // check-freshness.mjs unable to tell a real regen from a rerun. Falls
  // back to whatever is already committed, then to today, for the rare
  // case `git log` itself fails against a checked-out-but-broken sibling.
  const previousGeneratedAt = existsSync(outFile)
    ? readFileSync(outFile, "utf8").match(/generatedAt\s*=\s*"([^"]+)"/)?.[1]
    : undefined;
  const generatedAt =
    (() => {
      try {
        return execFileSync("git", ["log", "-1", "--format=%cs", "--", "docs/providers"], {
          cwd: join(reposRoot, "Android", "PaymentsLab"),
          env: gitEnv(),
          encoding: "utf8",
        }).trim();
      } catch {
    ...
    })() || previousGeneratedAt || new Date().toISOString().slice(0, 10);
