...
- **B5.** Hero elevation: fix particle-sphere/headline composition at ALL breakpoints; more expressive cursor reactivity; **land the deferred 3D upgrade here** — migrate `ParticleHeroScene` (and other R3F scenes) to Three `WebGPURenderer` (r171+, auto-fallback to WebGL2) / R3F v10 if stable at execution time (verify npm status then). This is the migration's explicitly-postponed WebGPU work.
...
- **D1.** HireSignal live: build `Repos/Android/HireSignal` wasmJs browser distribution (verify the exact Gradle task, e.g. `:composeApp:wasmJsBrowserDistribution`), copy to `public/hiresignal-app/`, add `liveUrl` to its `profile.ts` record + a WASM immutable-cache header in `vercel.json`, wire the intersection-gated live preview in `ProjectDetail`. Measure the added bundle weight.
...
- **E1.** LCP < 2.5s on SSR routes: defer/lazy the 3D hero so the headline is the LCP element and paints immediately (static poster → live sphere on idle/hydration). Then flip the LHCI `largest-contentful-paint` budget from `warn` back to `error`.
- **E2.** LHCI honesty: add `/blueprint` (or `/lab`) to the LHCI URL set so the `script:size` budget is actually exercised against the tldraw chunk.
