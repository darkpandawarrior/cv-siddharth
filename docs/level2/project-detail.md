...
6. Videos, if any.
7. "How it works" — `d.sections` rendered as a **symmetric grid of interchangeable cards**. This is where almost all the real engineering narrative already lives (Gaddi's "AI Munshi narrator," Doori's "Location engine," Stutter's "Record intent, never position" — genuinely excellent prose) but the grid layout gives every section equal visual weight, so nothing reads as *the* problem, *the* fork taken, or *the* payoff. It reads as a feature list, not a case study.
