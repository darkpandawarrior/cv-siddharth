...
describe("heavy()", () => {
  ...
  // concatenate straight into "//kursi-app/…" — a BROWSER parses a leading
  // "//" as protocol-relative (host "kursi-app"), not a same-origin path, so
  ...
  it("never produces a protocol-relative (leading //) URL", () => {
    expect(heavy("/kursi-app/index.html")).not.toMatch(/^\/\//);
  ...
  it("still joins onto a non-slash base (the production case) untouched", () => {
    expect(heavy("/kursi-app/index.html")).toContain("/kursi-app/index.html");
  });
});
