...
import { applyPromptGuard, fenceUntrusted, guard } from "./promptFence";

// This is the fence's real test suite, at the module it now lives in
// (idea-atlas SYS-9: the pure functions moved here so GuardLab.tsx can run
// the production fence in the browser). api/_lib/prompt-guard.test.ts stays
// as-is, unchanged, and now doubles as an integration check that the edge
// re-export still wires to the same behaviour.
