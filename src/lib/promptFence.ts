 ...
 *
 * Applied server-side in chat-handler.ts (via api/_lib/prompt-guard.ts, which
 * re-exports everything here) to the newest user turn only. History the
 * model already answered under stays as it was sent.
 *
 * Pure and dependency-free on purpose: GuardLab.tsx imports this module
 * directly and runs the SAME function in the browser, so the lab exercises
 * the production fence rather than a copy of it. No fetch, no React, no
 * server-only import belongs in this file.
 */
...
export type GuardMode = "chat" | "jd" | "compose";
