...
/**
 * Joins a root-relative path (e.g. "/gaddi-app/index.html") onto the base.
 *
 ...
 * two gives "//gaddi-app/…", which a BROWSER parses as protocol-relative
 * (host "gaddi-app", not a path on this origin) and fails to fetch. Node/curl
 ...
 * project's CURRENT slug. Gaddi, Doori and Stutter's `heavy/` folders were
 * renamed to match (`gaddi-app`, `doori-app`, `stutter-app`) alongside the
 * 2026-09-05 portfolio rename. `paymentslab-app` and `portfolio-app` keep
 * their original names — renaming THOSE still cascades into
 * `publish-heavy-assets.mjs`'s sibling-repo sync and `docs/perf-budgets.md`'s
 * measurements, neither of which this file owns — so those two slugs stay
 * internal technical identifiers, same rationale as the published
 * `applicationId`s that also outlive the rename. Every caller of `heavy()`
 * reads the real path through this map instead of typing the folder name
 * inline.
 */
export const WASM_APP_PATH = {
  doori: "/doori-app/index.html",
  gaddi: "/gaddi-app/index.html",
  paymentsLabKmp: "/paymentslab-app/index.html",
  portfolio: "/portfolio-app/index.html",
  stutter: "/stutter-app/index.html",
} as const;
