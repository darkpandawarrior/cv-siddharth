...
/**
 * The name each device frame goes by on a tile.
 *
 * It carried `aspect` and `width` too, for a poster pipeline that no longer
 * exists: SurfaceWall dropped the device band (its own docstring says why),
 * which left gen-surfaces.mjs cropping captures nothing rendered and
 * capture-site.mjs shooting a second per-device pass only that generator read.
 * All three are deleted. What survives is the claim the band was making, which
 * the wall still prints in words: this room is best seen on a phone.
 */
export const DEVICE: Record<DeviceFrame, { label: string }> = {
  phone: { label: "Phone" },
...
const roomSurfaces: SurfaceInput[] = [
  ...
  },
  {
    ...
    // them.
    blurb: `${countWord(LAB_TABS.length)} experiments that prove the numbers, running in your browser: Dice.tech production metrics, five personal builds and seven years of chess.`,
    tag: "canvas · physics",
    group: "proof",
    tint: ACCENT,
    device: "desktop",
  ...
  },
  {
    ...
    // all still reach it, so putting it back on the wall is one deleted line.
    wall: false,
  },
  {
    to: "/terminal",
  ...
  // which imports the generated chess.ts.
  {
    ...
    label: "The Board",
    blurb:
      "Seven years of games across lichess and chess.com, mined: the rating arc in 3D, where games end, a shifting repertoire, and a bot trained on the openings you are looking at.",
    tag: "3d · engine",
    group: "corpus",
...
const pageSurfaces: SurfaceInput[] = [
  ...
  },
  {
    ...
    label: "The Shelf",
    blurb:
      "Every Android app that reached the Play Store from this work: three at Dice and Jugnoo, the rest white-label clients of the same platform, each one verified against its live listing.",
    tag: "store · verified",
    group: "proof",
    tint: ACCENT,
    device: "phone",
