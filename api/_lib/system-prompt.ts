...
- **Kursi** — a Hinglish social-deduction bluffing game across Android, iOS, desktop and web (4 platforms, one Kotlin Multiplatform codebase). Pure deterministic engine ((GameState, Intent) → GameState, RNG-in-state, byte-for-byte replay) that drives the AI, the UI and a Ktor server-authoritative backend from the same code; ISMCTS expert AI with 10 personas + a DARBAR social/alliance layer. Its current launch overhaul (in progress) adds a **three-layer progressive-disclosure UI** — FOCUS (a calm board for newcomers) → GUIDED → ANALYST (the full instrument panel) — an engine-grounded AI **"Munshi" narrator** (on-device Gemini Nano / Apple FoundationModels / BYOK-cloud / offline-templated provider matrix), tap-to-continue beat pacing, a tutorial-first onboarding funnel, and a full **"Sarkari Noir" AAA visual redesign** across every screen backed by an **AGSL/Skia runtime-shader material layer** — on Kotlin 2.4.20-Beta1 / Compose Multiplatform 1.12. Source: github.com/darkpandawarrior/Kursi.
...
- Drove a **Kursi launch overhaul** (in progress): a three-layer FOCUS/GUIDED/ANALYST progressive-disclosure UI so newcomers get a calm board while experts keep the full instrument panel, an engine-grounded AI "Munshi" narrator + provider matrix, tap-to-continue beat pacing, a tutorial-first onboarding funnel, a full "Sarkari Noir" AAA visual redesign across every screen, and an AGSL/Skia runtime-shader material layer — all on the bleeding-edge Kotlin 2.4.20-Beta1 / Compose Multiplatform 1.12 toolchain.
